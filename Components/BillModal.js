...
export default RepModal = (props) => {
    ...
    const focusedBill = state.bills.reduce((acc, item) => item.billId === state.focusedBillId ? item : acc)

    console.log(focusedBill)
    actionsList = () => {

        return (
            focusedBill.actions.map((item, index) =>

                <ActionView key={index} theme={state.currentUser.theme} >
                    <BioText theme={state.currentUser.theme}> {item.action} </BioText>
                    <BioText theme={state.currentUser.theme}> {item.actionBy} </BioText>
                    <BioText theme={state.currentUser.theme}> {item.date} </BioText>
                </ActionView>
                ...
                <RepContainer theme={state.currentUser.theme}>
                    ...
                    >
                        ...
                        <RepBasicInfo theme={state.currentUser.theme}>
                            <BasicInfoText theme={state.currentUser.theme}>
                                {focusedBill.billId}</BasicInfoText>
                        </RepBasicInfo>
                        <SectionName theme={state.currentUser.theme}>Title</SectionName>
                        <RepBio theme={state.currentUser.theme}>
                            <BioText theme={state.currentUser.theme}> {focusedBill.billTitle} </BioText>
                        </RepBio>
                        <SectionName theme={state.currentUser.theme} >Actions</SectionName>
                        <ActionContainer>
                            {actionsList()}
                        </ActionContainer>
